...
你用 Cursor、Codex、Claude Code 等工具做了一个 AI 应用——翻译服务、聊天机器人、文档总结。它调用 OpenAI、DeepSeek 或任何 LLM API。你准备把链接发给朋友、微信群或者小团队试用。链接发出去之前，下面这些才是你真正需要知道的：

- 5 个朋友同时打开。有人看到报错吗？
- 有人粘贴了一篇长文档。请求会不会超时？
- AI 回复感觉很慢。是你代码的问题、$6 VPS 的问题，还是模型 API 的问题？
- 你在用 DeepSeek 的免费额度。是不是已经在触发限流了？
- 流式输出看着一顿一顿的。这是正常的还是哪里坏了？

这些不是"压测"问题，是"这个链接现在能不能发"的问题。
vibeready 一条命令回答它们。
